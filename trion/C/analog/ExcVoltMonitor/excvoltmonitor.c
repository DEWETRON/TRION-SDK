 ...
 * This example will only work on TRION-MULTI boards
 *
 * Describes following:
...
int main(int argc, char* argv[])
{
    ...
    int i=0;
    int nBoardID=0;
    const double fRangeMin = -33;//[V]
    const double fRangeMax = 15; //[V]
    ...
    snprintf(sSettingStr, sizeof(sSettingStr),"%s/AI3", sBoardID);
    nErrorCode = DeWeSetParamStruct_str( sSettingStr , "Mode", "ExcVoltMonitor");
    ...
    nErrorCode = DeWeSetParamStruct_str(sSettingStr, "IIRFilter_Val", "300 Hz");
    CheckError(nErrorCode);


    dumpAICurrentSettings( nBoardID, 3 );
    ...
    if (nErrorCode <= 0)
    {
        ...
        const  char* Excitations[] = {  "0.05 mA", "0.1 mA", "0.2 mA", "0.5 mA", "1 mA", "1.5 mA", "2 mA", "4 mA", "5 mA", "10 mA",      //0..9
        ...
        while( !kbhit() )
        {
            ...
            for (i = 0; i < nAvailSamples; ++i)
            {
                ...
                if ( 0 == i%100 )
                {
                    printf("%8.8X =  %#10.6f V\r", nRawData, fScaledVal);
                }
    ...
    )
{
    ...
    dumpProperty(sTargetString, "IIRFilter_Val");
