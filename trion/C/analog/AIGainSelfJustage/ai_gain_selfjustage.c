/**
 * Short example to describe how to trigger an gain-self-justage
 *
 * for analogue channels of a specific board
 *
 *
 * In this example no application-owned acquisition is running
 * The justage-operation will execute blocking, and will not
 * return until finished
 *
 * After execution the new gain-correction values are already
 * written to the e2prom data of the board
...
int main(int argc, char* argv[])
{
    ...
    nErrorCode = DeWeSetParamStruct_str( sChannelStr, "SelfJustage", "");
    CheckError(nErrorCode);

    // so the final result is available with the next line of code
    nErrorCode = DeWeGetParamStruct_str( sChannelStr, "SelfJustage", sSettingStr, sizeof(sSettingStr));
    CheckError(nErrorCode);
    if ( nErrorCode > 0 ) 
    {
        //Error - Trap
        printf("Error during gain self justage: %s\nAborting.....\n", DeWeErrorConstantToString(nErrorCode));
    }
    else
    {
        ...
        printf("\n\nGain Selfjustage finished for Board %d\nThe gain-correction values are written to E2Prom\n", nBoardID);
    };
