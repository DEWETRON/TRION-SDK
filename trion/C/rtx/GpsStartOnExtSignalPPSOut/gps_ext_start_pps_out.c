 ...
 *  - Set AUX on chassis controller to output PPS
...
int run_example()
{
    ...
    for (board_no = 0; board_no < NUM_BOARDS; ++board_no)
    {
        ...
        if (master_board == board_no)
        {
            ...
            // Set start counter to 1 to start on 2nd SYNC0 signal (1st is Sync, 2nd Start)
            nErrorCode = DeWeSetParamStruct_str(target, "StartCounter", "1");
            CheckError(nErrorCode);

            // Configure GPS for Time-information
            ...
            // Route the PPS signal to AUX
            // either PPS, which is the FPGA-generated PPS
			// or PPS_REC, which is the reference-PPS from the receiver
			// afer successfull syncing, those two should be practically the same
            snprintf(target, sizeof(target), "BoardID%d/AUX", board_no);
            nErrorCode = DeWeSetParamStruct_str(target, "Source", "PPS");
            CheckError(nErrorCode);
            nErrorCode = DeWeSetParamStruct_str(target, "Inverted", "False");
            CheckError(nErrorCode);
