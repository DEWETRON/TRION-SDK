   ...
   if ( CheckError(nErrorCode))
   {
    ...
    if (nErrorCode <= 0)
    {
        ...
        while( !kbhit() )
        {
            ...
            float timestamp_first = 0.0f;
            float timestamp_last = 0.0f;
            ...
            nAvailCanMsgs = 0;
            nErrorCode = DeWeReadCANRawFrame(nBoardID, &RawFrame, &nAvailCanMsgs );
            if ( CheckError(nErrorCode))
            {
            ...
            {
                timestamp = (float)((float)(RawFrame[i].Pos) / 10000000);  //Timestamp in 100ns re-formated to seconds
                if (0 == i)
                {
                    timestamp_first = timestamp;
#ifdef VERBOSE
                    printf("\nprocessing %d CAN messages...  ", nAvailCanMsgs);
#endif
                }
                if (nAvailCanMsgs - 1 == i)
                {
                    timestamp_last = timestamp;
#ifdef  VERBOSE
                    printf("covering %f msec.", (timestamp_last - timestamp_first) * 1000);
                    if (((timestamp_last - timestamp_first) * 1000) > 150){
                        printf("  unusual long time span (>150ms).");
                    }
#else //  VERBOSE
                    if (((timestamp_last - timestamp_first) * 1000) > 200){
                        printf("\nprocessed %d CAN messages...  ", nAvailCanMsgs);
                        printf("covering %f msec.", (timestamp_last - timestamp_first) * 1000);
                        printf("  unusual long time span (>200ms).");
                        printf("\n ts_first: %f", timestamp_first);
                        printf("\n ts_last: %f\n\n", timestamp_last);
                    }
#endif
                }
                ...
                printf("[%012.7f] HDR: %8.8X   Port: %d   Errorcount: %d   DataLen: %d   Data: %2.2X %2.2X %2.2X %2.2X   %2.2X %2.2X %2.2X %2.2X\n",
                            ...
                            ((RawFrame[i].Err >> 24) & 0x0F),
                            RawFrame[i].Data[0], RawFrame[i].Data[1], RawFrame[i].Data[2], RawFrame[i].Data[3],
                            RawFrame[i].Data[4], RawFrame[i].Data[5], RawFrame[i].Data[6], RawFrame[i].Data[7]
                        );
            }



            nErrorCode = DeWeFreeFramesCAN(nBoardID, nAvailCanMsgs);
            CheckError(nErrorCode);
