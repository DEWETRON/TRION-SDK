/**
 * Short example to showcase a temperature measurement on a TRION-MULTI Module
 *
...
int main(int argc, char* argv[])
{
    ...
    const double fRangeMin = -100;       // MAX MIN VALUE: -200�C
    const double fRangeMax = 200;        // MAX MAX VALUE: 850�C
    const double fExcitation = 0.5f;
    const int iSensRes0 = 600;
   ...
   // Get channel number form command line; Default number is "0"
    ...
    nErrorCode = DeWeGetParamStruct_str( sTarget , "SensorRes0", sResultStr, sizeof(sResultStr));
    CheckError(nErrorCode);
    res0_back = strtod(sResultStr, &punit);
    printf("SensorRes0 Value:   %f  %s\n", res0_back, punit);
    ...
    while( !kbhit() )
    {
        ...
        for (i = 0; i < nAvailSamples; ++i)
        {
            ...
            if (0 == i%10)
            {
                printf("\rRaw: %8.8x --> Scaled: %4.3f[degC]",nRawData, fVal);
                fflush(stdout);
            }
