...
int main(int argc, char* argv[])
{
    ...
    // leave all other AI channels (4,5 or 4,5,6,7 depending on connector-panel)
    // disabled
    snprintf(sChannelStr, sizeof(sChannelStr),"BoardID%d/AI0", nBoardID);
    nErrorCode = DeWeSetParamStruct_str( sChannelStr, "Used", "True");
    CheckError(nErrorCode);
    snprintf(sChannelStr, sizeof(sChannelStr),"BoardID%d/AI1", nBoardID);
    nErrorCode = DeWeSetParamStruct_str( sChannelStr, "Used", "True");
    CheckError(nErrorCode);
    snprintf(sChannelStr, sizeof(sChannelStr),"BoardID%d/AI2", nBoardID);
    nErrorCode = DeWeSetParamStruct_str( sChannelStr, "Used", "True");
    CheckError(nErrorCode);

    //prepare the acquisition parameters
    // Set configuration to use one board in standalone operation
    snprintf(sSettingStr, sizeof(sSettingStr),"BoardID%d/AcqProp", nBoardID);
    ...
    // For the samplerate 1000 samples per second, 100 is a buffer for
    // 0.1 seconds
